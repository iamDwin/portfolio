...
<template>
  <article
    class="group cursor-pointer overflow-hidden rounded-2xl border border-border/70 bg-card transition-all duration-500 hover:-translate-y-1 hover:border-accent/50 hover:shadow-glow"
    tabindex="0"
  ...
  >
    <div class="relative aspect-[16/10] overflow-hidden bg-elevated">
      ...
      <div class="absolute inset-0 bg-gradient-to-t from-[#111827]/35 to-transparent" />
      <a v-if="project.liveUrl" :href="project.liveUrl" target="_blank" rel="noopener noreferrer" aria-label="View live project" class="absolute right-4 top-4 flex h-10 w-10 items-center justify-center rounded-lg bg-void/80 text-text-primary backdrop-blur-md transition-colors hover:bg-accent hover:text-on-accent" @click.stop>
